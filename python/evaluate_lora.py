...
Usage:
    ...
    python evaluate_lora.py --export-tests tests.jsonl  # Export test cases
"""
...
# =============================================================================
# Installer-Specific Evaluator
# =============================================================================

class InstallerEvaluator(ModelEvaluator):
    """Model evaluator with installer-specific prompts and tools."""
    ...
    def __init__(self, model_path: str, adapter_path: str = None):
        ...
        super().__init__(
            ...
            tool_schema=SHELL_COMMAND_TOOL,
            dtype="float32",
        )


def export_test_cases(path: Path):
    """Export test cases to JSONL format for use with toolkit evaluator."""
    with open(path, "w") as f:
        for tc in TEST_CASES:
            f.write(json.dumps(tc) + "\n")
    print(f"Exported {len(TEST_CASES)} test cases to {path}")


def evaluate_adapter(model_path: Path, adapter_path: Path = None) -> EvalSummary:
    """Evaluate a single adapter."""
    evaluator = InstallerEvaluator(str(model_path), str(adapter_path) if adapter_path else None)
...
def main():
    ...
    parser.add_argument("--export-tests", default=None,
                        help="Export test cases to JSONL file")
    args = parser.parse_args()

    # Export mode
    if args.export_tests:
        export_test_cases(Path(args.export_tests))
        return
